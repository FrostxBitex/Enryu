...
// //   if (!(temp instanceof CanvasRenderingContext2D)) throw new Error("Assertion error");
// //   graphics = temp;
// // }
...
// //   else if (result === null) throw new Error("Element not found");
// //   else throw new TypeError("Invalid element type");
// // }
...
// //     const width: number = (canvas.width = baseImage.width);
// //     const height: number = (canvas.height = baseImage.height);
...
// //         const doShuffle = function (i: number): void {
...
// //             const j: number = i + Math.floor(Math.random() * (width - i));
// //             for (let y = 0; y < height; y++) {
// //               for (let x = j - 1; x >= i; x--) {
// //                 const off: number = (y * width + x) * 4;
// //                 for (let k = 0; k < 4; k++) {
// //                   const temp: number = pixels[off + k];
...
// //             }
// //             i++;
// //             if (Date.now() - startTime > YIELD_AFTER_TIME) break;
// //           }
// //           graphics.putImageData(shuffledImage, 0, 0);

// //           // Continue shuffling or finish
// //           if (i < width) timeout = setTimeout(doShuffle, 0, i);
// //           else {
...
// //         let curIteration: number = 0;
// //         let curEnergy: number = 0;
// //         let colPermutation: Array<number> = [];
...
// //             const i: number = columnDiffs.length;
// //             let entry = new Uint32Array(width);
// //             for (let j = 0; j < width; j++) {
// //               if (i <= j) entry[j] = lineDiff(pixels, width, height, i, j);
// //               else entry[j] = columnDiffs[j][i];
// //             }
...
// //           } else {
// //             for (let i = 0; i < width - 1; i++) curEnergy += columnDiffs[i][i + 1];
// //             for (let i = 0; i < width; i++) colPermutation.push(i);
// //             annealingLastDrawTime = Date.now();
// //             doAnneal();
// //           }
// //         }

// //         function lineDiff(pixels: Uint8ClampedArray, width: number, height: number, x0: number, x1: number): number {
// //           let sum: number = 0;
// //           for (let y = 0; y < height; y++) {
// //             const off0: number = (y * width + x0) * 4;
// //             const off1: number = (y * width + x1) * 4;
// //             for (let i = 0; i < 3; i++) sum += Math.abs(pixels[off0 + i] - pixels[off1 + i]);
// //           }
// //           return sum;
// //         }
...
// //           let perm: Array<number> = colPermutation;
...
// //             const col0: number = Math.floor(Math.random() * width);
// //             const col1: number = Math.floor(Math.random() * width);
// //             if (col0 != col1) {
// //               // Calculate the change in energy if the col0 were removed and inserted at col1
// //               let energyDiff: number = 0;
// //               if (col0 >= 1) energyDiff -= columnDiffs[perm[col0 - 1]][perm[col0]];
// //               if (col0 + 1 < width) energyDiff -= columnDiffs[perm[col0]][perm[col0 + 1]];
// //               if (col0 >= 1 && col0 + 1 < width) energyDiff += columnDiffs[perm[col0 - 1]][perm[col0 + 1]];
// //               if (col1 < col0) {
// //                 if (col1 >= 1) {
...
// //                 const off0: number = (y * width + perm[x]) * 4;
// //                 const off1: number = (y * width + x) * 4;
// //                 for (let i = 0; i < 4; i++) annealedPixels[off1 + i] = shuffledPixels[off0 + i];
// //               }
// //             }
...
// //           if (curIteration < numIterations) timeout = setTimeout(doAnneal);
// //           else setButtonsBusy(false);
// //         }

// //         function formatWithThousandsSeparators(n: number): string {
// //           let s: string = n.toString();
// //           for (let i = s.length - 3; i > 0; i -= 3) s = s.substring(0, i) + " " + s.substring(i);
// //           return s;
// //         }
// //       }
// //     }
...
// function startAnneal(): void {
//   const numIterations: number = Math.round(30 * 1e6);
//   if (numIterations <= 0) {
//     alert("Number of iterations must be positive");
//     return;
//   }
//   const startTemperature: number = 4000;
...
//   let colPermutation: Array<number> = [];
//   let annealingLastDrawTime: number | null = null;
//   const width: number = 1115;
//   const height: number = 1600;
//   let columnDiffs: Array<Uint32Array> = [];
//   doAnnealPrecompute();
